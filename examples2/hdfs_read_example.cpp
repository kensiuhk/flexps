...
void Run() {
  ...
  Coordinator* coordinator =
      new Coordinator(my_node.id, config.worker_host, zmq_context, config.master_host, config.master_port);
  coordinator->serve();

  load_thread = std::thread([config, coordinator, nodes, my_node, zmq_context] {
    auto start_time = std::chrono::steady_clock::now();
    int proc_id = my_node.id;
    LineInputFormat infmt(config.input, nodes.size(), 0, coordinator, config.worker_host, config.hdfs_namenode,
