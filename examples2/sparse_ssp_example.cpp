...
void Run() {
  ...
  CHECK(FLAGS_kModelType == "ASP" || FLAGS_kModelType == "BSP" || FLAGS_kModelType == "SSP" ||
        FLAGS_kModelType == "SparseSSP");
  CHECK(FLAGS_kSparseSSPRecorderType == "None" || FLAGS_kSparseSSPRecorderType == "Map" ||
        FLAGS_kSparseSSPRecorderType == "Vector");
  ...
  if (model_type == ModelType::SparseSSP) {
    engine.CreateSparseSSPTable<float>(kTableId, range, model_type, storage_type, FLAGS_kStaleness, FLAGS_kSpeculation,
  ...
  task.SetLambda([kTableId](const Info& info) {
    ...
    std::vector<third_party::SArray<Key>> future_keys =
        GenerateRandomKeys(FLAGS_num_nonzeros, FLAGS_num_dims, FLAGS_num_iters + FLAGS_kSpeculation);
    // std::vector<third_party::SArray<Key>> future_keys = GenerateFixedKeys(FLAGS_num_iters+FLAGS_kSpeculation,
    // info.worker_id);
    // std::vector<third_party::SArray<Key>> future_keys = GenerateAllKeys(FLAGS_num_dims,
    // FLAGS_num_iters+FLAGS_kSpeculation);
