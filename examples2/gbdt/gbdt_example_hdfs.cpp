...
std::vector<std::string> load_data_from_hdfs(Node& my_node, std::vector<Node>& nodes, std::string input_path) {
  ...
  hdfs_manager.Run([my_node, &data, &mylock](HDFSManager::InputFormat* input_format, int local_tid) {
    ...
    LOG(INFO) << data.size() << " lines in (node, thread):(" << my_node.id << "," << local_tid << ")";
  });
  hdfs_manager.Stop();
...
void distribute_data_to_worker(int data_num, int worker_id, int worker_num, std::vector<float>& all_class_vect,
                               std::vector<float>& class_vect, std::vector<std::vector<float>>& all_feat_vect_list,
  ...
  if (worker_id == worker_num - 1) {
    ...
    for (int i = 0; i < feat_vect_list.size(); i++) {
      feat_vect_list[i].insert(feat_vect_list[i].end(),
                               all_feat_vect_list[i].begin() + (worker_id * num_of_records_per_worker),
                               all_feat_vect_list[i].end());
    }
  } else {
    class_vect.insert(class_vect.end(), all_class_vect.begin() + (worker_id * num_of_records_per_worker),
...
void Run() {
  ...
  std::vector<std::map<std::string, float>> global_min_max_feat_list =
      channel_for_global_min_max_feat(min_max_feat_list, my_node, nodes);
  ...
  std::map<std::string, int> kv_table_name_to_tid = {
      {"quantile_sketch", 0}, {"grad_and_hess", 1}, {"grad_sum_and_count", 2}};
  ...
  task.SetLambda([nodeId, worker_num, &all_predict_result, &kv_table_name_to_tid, &train_data_loader, &test_data_loader,
                  &global_min_max_feat_list, &global_data_num](const Info& info) {
    ...
    distribute_data_to_worker(data_num, info.local_id, worker_num, all_class_vect, class_vect, all_feat_vect_list,
                              feat_vect_list);

    // Step 0: Initialize setting
    std::map<std::string, std::unique_ptr<KVClientTable<float>>> name_to_kv_table;
    for (std::map<std::string, int>::iterator it = kv_table_name_to_tid.begin(); it != kv_table_name_to_tid.end();
         it++) {
    ...
    std::map<std::string, float> params = {{"num_of_trees", FLAGS_num_of_trees},
    ...
    std::map<std::string, std::string> options = {{"loss_function", "square_error"}};
