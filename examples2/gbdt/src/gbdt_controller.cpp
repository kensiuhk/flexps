...
GBDTController::GBDTController() { this->ps_key_ptr = 0; }
...
GBDTTree GBDTController::build_tree(int tree_id,
  ...
  tree.train(this->ps_key_ptr, name_to_kv_table, feat_vect_list, min_max_feat_list, grad_vect, hess_vect,
             computation_time, communication_time);

  return tree;
}

GBDTTree GBDTController::create_tree() { return GBDTTree(this->params); }

void GBDTController::build_forest(std::map<std::string, std::unique_ptr<KVClientTable<float>>>& name_to_kv_table,
...
void GBDTController::update_estimator_vect(GBDTTree& tree, std::vector<float>& estimator_vect,
...
void GBDTController::calculate_predict_result(std::vector<std::vector<float>>& test_feat_vect_list,
                                              std::vector<float>& test_class_vect,
...
// Helper function
std::vector<float> GBDTController::get_gradient_vect(std::vector<float>& class_vect, std::vector<float>& estimator_vect,
                                                     std::string loss_function, int order) {
