...
void channel_for_balancing_hdfs_data(DataLoader& train_data_loader, int& global_data_num, Node& my_node,
  ...
  std::thread th = std::thread([local_channels, num_global_threads, &train_data_loader, &global_data_num]() {
    ...
    const std::vector<int> msg{lc->GetId(), train_data_loader.get_class_vect().size()};
    send_bin << msg;
    ...
    if (lc->GetId() == 0) {
      ...
      const std::vector<int> msg{global_data_num};
      send_bin << msg;
...
std::vector<std::map<std::string, float>> channel_for_global_min_max_feat(
    std::vector<std::map<std::string, float>>& min_max_feat_list, Node& my_node, std::vector<Node>& nodes) {
  ...
  std::thread th = std::thread([local_channels, num_global_threads, &global_min_max_feat_list, &min_max_feat_list]() {
    ...
    for (int j = 0; j < min_max_feat_list.size(); j++) {
      ...
      const std::vector<float> msg{min_max_feat_list[j]["min"], min_max_feat_list[j]["max"]};
    ...
    if (lc->GetId() == 0) {
      ...
      for (int j = 0; j < global_min_max_feat_list.size(); j++) {
        const std::vector<float> msg{global_min_max_feat_list[j]["min"], global_min_max_feat_list[j]["max"]};
        send_bin << msg;
      }
...
void channel_for_global_predict_result(const std::map<std::string, float>& local_predict_result, Node& my_node,
                                       std::vector<Node>& nodes) {
  // 1. Manually start id_mapper and mailbox
  SimpleIdMapper id_mapper(my_node, nodes);
  id_mapper.Init();
  ...
  for (int i = 0; i < threads.size(); ++i) {
    threads[i] = std::thread([i, local_channels, num_global_threads, &local_predict_result]() {
      ...
      const std::vector<float> msg{local_predict_result.at("sse"), local_predict_result.at("num")};
