...
void GBDTTree::train(int& ps_key_ptr, std::map<std::string, std::unique_ptr<KVClientTable<float>>>& name_to_kv_table,
                     std::vector<std::vector<float>>& feat_vect_list,
                     std::vector<std::map<std::string, float>>& min_max_feat_list, std::vector<float>& grad_vect,
                     std::vector<float>& hess_vect, float& computation_time, float& communication_time) {
  ...
  for (int f_id = 0; f_id < feat_vect_list.size(); f_id++) {
    push_key_vect =
        push_quantile_sketch(ps_key_ptr, table, feat_vect_list[f_id], min_max_feat_list[f_id], push_val_vect);
  ...
  ;
  computation_time +=
      std::chrono::duration<double, std::milli>(computation_end_time - computation_start_time).count() / 1000;
  ...
  ;
  communication_time +=
      std::chrono::duration<double, std::milli>(communication_end_time - communication_start_time).count() / 1000;
  ...
  for (int f_id = 0; f_id < feat_vect_list.size(); f_id++) {
    std::vector<float> sketch_hist_vect(pull_val_vect.begin() + (f_id * sketch_num_per_feat),
                                        pull_val_vect.begin() + ((f_id + 1) * sketch_num_per_feat));
  ...
  for (int f_id = 0; f_id < feat_vect_list.size(); f_id++) {
    ...
    push_key_vect = push_local_grad_hess(ps_key_ptr, table, feat_vect_list[f_id], candidate_split_vect_list[f_id],
                                         grad_vect, hess_vect, push_val_vect);
  ...
  for (int f_id = 0; f_id < feat_vect_list.size(); f_id++) {
    std::vector<float> grad_hess_vect(pull_val_vect.begin() + (f_id * grad_hess_num_per_feat),
                                      pull_val_vect.begin() + ((f_id + 1) * grad_hess_num_per_feat));
  ...
  for (int i = 0; i < this->quantile_sketch_key_vect_list.size(); i++) {
    ...
    aggr_push_key_vect.insert(aggr_push_key_vect.end(), quantile_sketch_key_vect_list[i].begin(),
                              quantile_sketch_key_vect_list[i].end());
    aggr_push_val_vect.insert(aggr_push_val_vect.end(), push_val_vect.begin(), push_val_vect.end());
  }
  ...
  for (int i = 0; i < this->grad_hess_key_vect_list.size(); i++) {
    ...
    push_val_vect = inv_val_vect;
    aggr_push_key_vect.insert(aggr_push_key_vect.end(), grad_hess_key_vect_list[i].begin(),
                              grad_hess_key_vect_list[i].end());
    aggr_push_val_vect.insert(aggr_push_val_vect.end(), push_val_vect.begin(), push_val_vect.end());
  }
  ...
  push_val_vect = inv_val_vect;
  aggr_push_key_vect.insert(aggr_push_key_vect.end(), local_grad_sum_key_vect_list.begin(),
                            local_grad_sum_key_vect_list.end());
  aggr_push_val_vect.insert(aggr_push_val_vect.end(), push_val_vect.begin(), push_val_vect.end());
  ...
  DataLoader::split_dataset_by_feat_val(this->feat_id, this->split_val, grad_vect, left_grad_vect, right_grad_vect,
                                        hess_vect, left_hess_vect, right_hess_vect, feat_vect_list, left_feat_vect_list,
  ...
  this->left_child->train(ps_key_ptr, name_to_kv_table, left_feat_vect_list, left_min_max_feat_list, left_grad_vect,
                          left_hess_vect, computation_time, communication_time);
  this->right_child->train(ps_key_ptr, name_to_kv_table, right_feat_vect_list, right_min_max_feat_list, right_grad_vect,
                           right_hess_vect, computation_time, communication_time);
}

std::vector<Key> GBDTTree::push_quantile_sketch(int& ps_key_ptr, std::unique_ptr<KVClientTable<float>>& table,
                                                std::vector<float>& feat_vect,
                                                std::map<std::string, float>& min_max_feat,
                                                std::vector<float>& _push_val_vect) {
...
std::vector<float> GBDTTree::find_candidate_split(std::vector<float>& sketch_hist_vect,
                                                  std::map<std::string, float>& min_max_feat) {
  float rank_fraction = this->params["rank_fraction"];
  float total_data_num = this->params["total_data_num"];
  float min = min_max_feat["min"];
  ...
  for (int c_id = 0; c_id < rank_fraction_vect.size(); c_id++) {
    ...
    for (int s_id = 0; s_id < sketch_candidate_range; s_id++) {
      ...
      float cur_fraction_sum = std::accumulate(sketch_hist_vect.begin(),
                                               sketch_hist_vect.begin() + (int) (cur_fraction / sketch_fraction), 0.0);
...
std::vector<Key> GBDTTree::push_local_grad_hess(int& ps_key_ptr, std::unique_ptr<KVClientTable<float>>& table,
                                                std::vector<float>& feat_vect, std::vector<float>& candidate_split_vect,
...
std::map<std::string, float> GBDTTree::find_best_split(std::vector<float>& grad_hess_vect, float complexity_of_leaf) {
  ...
  std::vector<float> left_hess_val_vect(grad_hess_vect.begin() + (grad_hess_vect.size() / 4),
                                        grad_hess_vect.begin() + (grad_hess_vect.size() / 2));
  std::vector<float> right_grad_val_vect(grad_hess_vect.begin() + (grad_hess_vect.size() / 2),
                                         grad_hess_vect.begin() + (grad_hess_vect.size() * 3 / 4));
  std::vector<float> right_hess_val_vect(grad_hess_vect.begin() + (grad_hess_vect.size() * 3 / 4),
                                         grad_hess_vect.begin() + grad_hess_vect.size());
...
std::vector<Key> GBDTTree::push_local_grad_sum(int& ps_key_ptr, std::unique_ptr<KVClientTable<float>>& table,
                                               std::vector<float>& grad_vect, std::vector<float>& _push_val_vect) {
...
void GBDTTree::set_depth(int depth) { this->depth = depth; }
